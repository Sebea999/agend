 ...
 * @author RYUU
 */
@WebServlet(name="pdf_atencion_receta", urlPatterns="/receta_pdf")
public class PdfAtencion_Receta extends HttpServlet {
...
//                }
                ...
                // RECUPERO EL CAMPO DE TEXTO DE RECETA 
                String txt_receta = (String) request.getParameter("tAR");
                System.out.println("_   _PDF___RECETA_TXT:     :"+txt_receta);
                ...
                documento.addTitle("Detalle de la Receta");
...
//                documento.add(Chunk.NEWLINE); // LINEA EN BLANCO // ENTER 
                // CARGO EL SUBTITULO ----------------
                Paragraph subtitulo = new Paragraph("Detalles de la Receta", FontFactory.getFont("arial", 15, Font.BOLD));
...
//                tableDetalle.addCell(valueIdPaciente); // AÑADO A LA TABLA LA CELDA 
                ...
                // LABEL, RECETA 
                PdfPCell lblReceta = new PdfPCell(new Phrase("Receta: ", FontFactory.getFont("arial", 12, Font.BOLD)));
                lblReceta.setVerticalAlignment(Element.ALIGN_TOP); // centrado vertical 
                lblReceta.setHorizontalAlignment(Element.ALIGN_RIGHT); // centrado horizontal 
                lblReceta.setPaddingTop(valor10f);
                lblReceta.setPaddingRight(valor20f);
                lblReceta.setBorder(Rectangle.NO_BORDER);
                tableDetalle.addCell(lblReceta); // AÑADO A LA TABLA LA CELDA 
                // VALOR DEL DATO, RECETA 
                PdfPCell valueReceta = new PdfPCell(new Phrase(txt_receta));
                valueReceta.setVerticalAlignment(Element.ALIGN_MIDDLE); // centrado vertical 
                valueReceta.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
                valueReceta.setPaddingTop(valor10f);
                valueReceta.setBorder(Rectangle.NO_BORDER);
                tableDetalle.addCell(valueReceta); // AÑADO A LA TABLA LA CELDA 
