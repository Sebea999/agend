 ...
 * @author RYUU
 */
@WebServlet(name="pdf_atencion_estudios", urlPatterns="/estudios_solicitados_pdf")
public class PdfAtencion_EstudiosSoli extends HttpServlet {
...
//                }
                ...
                // RECUPERO EL CAMPO DE TEXTO DE ESTUDIOS SOLICITADOS/REALIZADOS 
                String txt_est_soli = (String) request.getParameter("tAES");
                System.out.println("_   _PDF___ESTUDIOS_SOLI_/_REALIZADOS_TXT:     :"+txt_est_soli);
                ...
                documento.addTitle("Detalle de los Estudios Solicitados");
...
//                documento.add(Chunk.NEWLINE); // LINEA EN BLANCO // ENTER 
                // CARGO EL SUBTITULO 
                Paragraph subtitulo = new Paragraph("Detalles de los Estudios Solicitados / Realizados", FontFactory.getFont("arial", 15, Font.BOLD));
...
//                tableDetalle.addCell(valueIdPaciente); // AÑADO A LA TABLA LA CELDA 
                ...
                // LABEL, ESTUDIOS_SOLICITADOS/REALIZADOS 
                PdfPCell lblEstudiosSoli = new PdfPCell(new Phrase("Estudios Solicitados/Realizados: ", FontFactory.getFont("arial", 12, Font.BOLD)));
                lblEstudiosSoli.setVerticalAlignment(Element.ALIGN_TOP); // centrado vertical 
                lblEstudiosSoli.setHorizontalAlignment(Element.ALIGN_RIGHT); // centrado horizontal 
                lblEstudiosSoli.setPaddingTop(valor10f);
                lblEstudiosSoli.setPaddingRight(valor20f);
                lblEstudiosSoli.setBorder(Rectangle.NO_BORDER);
                tableDetalle.addCell(lblEstudiosSoli); // AÑADO A LA TABLA LA CELDA 
                // VALOR DEL DATO, ESTUDIOS_SOLICITADOS/REALIZADOS 
                PdfPCell valueEstSoli = new PdfPCell(new Phrase(txt_est_soli));
                valueEstSoli.setVerticalAlignment(Element.ALIGN_MIDDLE); // centrado vertical 
                valueEstSoli.setHorizontalAlignment(Element.ALIGN_JUSTIFIED);
                valueEstSoli.setPaddingTop(valor10f);
                valueEstSoli.setBorder(Rectangle.NO_BORDER);
                tableDetalle.addCell(valueEstSoli); // AÑADO A LA TABLA LA CELDA 
