...
public class ModelMedico  implements CRUD {
    ...
    private String IDCATEG_PER_MEDICO = "5"; // ID 5 = MEDICO
    ...
    */
    private Connection devolverConex() {
        System.out.println("[+] MODEL_MEDICO.-------");
        try {
...
//            Class.forName("com.mysql.jdbc.Driver");//Se registra la direccion dentro del .jar el Driver jdbc de MySQL
            ...
            MM_CONEXION = java.sql.DriverManager.getConnection((host+bd+configdb), user, pass);
//            MM_CONEXION = java.sql.DriverManager.getConnection((host+bd+"?autoReconnect=true&useTimeZone=true&serverTimezone=UTC&useSSL=false"), user, pass);
            System.out.println("[+] Connection is successful.-");
        } catch (ClassNotFoundException | SQLException ex) {
            System.out.println("[-] Connection is failed --- class-not-found-exception | sql-exception.-");
            Logger.getLogger(ModelMedico.class.getName()).log(Level.SEVERE, null, ex);
        }
        return MM_CONEXION;
    } // END METHOD  
    ...
    ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    */
    private static Connection MM_CONEXION = null;
    private static Statement MM_SENTENCIA = null;
    private static ResultSet MM_RESULTADO = null;
...
//            }
            MM_SENTENCIA = MM_CONEXION.createStatement();
...
//            }
            MM_RESULTADO = MM_SENTENCIA.executeQuery(sql);
    ...
    public List cargar_grilla(HttpSession PARAM_SESION) {
        ...
        try {
            String sql = "SELECT rp.IDPERSONA, rp.NOMBRE, rp.APELLIDO, rp.CINRO, rp.TIPODOC, rp.IDCATEGORIA_PERSONA, rp.DESC_CATEG_PERSONA, \n" +
                ...
                "AND rp.IDCATEGORIA_PERSONA = "+IDCATEG_PER_MEDICO+" \n" +
            ...
            MM_RESULTADO = consultaBD(sql);
            
            while(MM_RESULTADO.next()) {
                BeanPersona datos = new BeanPersona();
                    datos.setRP_IDPERSONA(MM_RESULTADO.getString("IDPERSONA"));
                    datos.setRP_NOMBRE(MM_RESULTADO.getString("NOMBRE"));
                    datos.setRP_APELLIDO(MM_RESULTADO.getString("APELLIDO"));
                    datos.setRP_CINRO(MM_RESULTADO.getString("CINRO"));
                    datos.setRP_IDCLINICA(MM_RESULTADO.getString("IDCLINICA"));
                    if (MM_RESULTADO.getString("TIPODOC") != null) {
                        TIPODOC = MM_RESULTADO.getString("TIPODOC");
                    } else {
                        TIPODOC = "";
                    }
                    datos.setRP_TIPODOC(TIPODOC);
                    if (MM_RESULTADO.getString("IDCATEGORIA_PERSONA") != null) {
                        IDCATEGORIA_PERSONA = MM_RESULTADO.getString("IDCATEGORIA_PERSONA");
                    } else {
                        IDCATEGORIA_PERSONA = "";
                    }
                    datos.setRP_IDCATEGORIA_PERSONA(IDCATEGORIA_PERSONA);
                    if (MM_RESULTADO.getString("DESC_CATEG_PERSONA") != null) {
                        DESC_CATEG_PERSONA = MM_RESULTADO.getString("DESC_CATEG_PERSONA");
                    } else {
                        DESC_CATEG_PERSONA = "";
                    }
                    datos.setRP_DESC_CATEG_PERSONA(DESC_CATEG_PERSONA);
                    if (MM_RESULTADO.getString("RAZON_SOCIAL") != null) {
                        RAZON_SOCIAL = MM_RESULTADO.getString("RAZON_SOCIAL");
                    } else {
                        RAZON_SOCIAL = "";
                    }
                    datos.setRP_RAZON_SOCIAL(RAZON_SOCIAL);
                    if (MM_RESULTADO.getString("RUC") != null) {
                        RUC = MM_RESULTADO.getString("RUC");
                    } else {
                        RUC = "";
                    }
                    datos.setRP_RUC(RUC);
                    if (MM_RESULTADO.getString("DIRECCION") != null) {
                        DIRECCION = MM_RESULTADO.getString("DIRECCION");
                    } else {
                        DIRECCION = "";
                    }
                    datos.setRP_DIRECCION(DIRECCION);
                    if (MM_RESULTADO.getString("EMAIL") != null) {
                        EMAIL = MM_RESULTADO.getString("EMAIL");
                    } else {
                        EMAIL = "";
                    }
                    datos.setRP_EMAIL(EMAIL);
                    if (MM_RESULTADO.getString("TELFPARTICULAR") != null) {
                        TELFPARTICULAR = MM_RESULTADO.getString("TELFPARTICULAR");
                    } else {
                        TELFPARTICULAR = "";
                    }
                    datos.setRP_TELFPARTICULAR(TELFPARTICULAR);
                    if (MM_RESULTADO.getString("TELFMOVIL") != null) {
                        TELFMOVIL = MM_RESULTADO.getString("TELFMOVIL");
                    } else {
                        TELFMOVIL = "";
                    }
                    datos.setRP_TELFMOVIL(TELFMOVIL);
                    if (MM_RESULTADO.getString("TELEFPARTICULAR") != null) {
                        TELEFPARTICULAR = MM_RESULTADO.getString("TELEFPARTICULAR");
                    } else {
                        TELEFPARTICULAR = "";
                    }
            ...
            Logger.getLogger(ModelMedico.class.getName()).log(Level.SEVERE, null, e);
        }
        return lista;
    }
...
//        System.out.println("_   __CANTIDAD_MIN_MOSTRAR:     :"+cant_min);
        ...
        if (sesionDatos.getAttribute("PAG_MED_CANT_BTN_DERE_CLIC") == null || String.valueOf(sesionDatos.getAttribute("PAG_MED_CANT_BTN_DERE_CLIC")).isEmpty()) {
            CANT_CLICS_DERECHO = 1;
        } else {
            CANT_CLICS_DERECHO = Integer.parseInt(String.valueOf(sesionDatos.getAttribute("PAG_MED_CANT_BTN_DERE_CLIC")));
        }
...
//                    System.out.println("__*____LISTA_MOSTRAR___( "+PARAM_NRO_PAG_MOSTRAR+" )___");
                    ...
                    BeanPersona datos = new BeanPersona();
                        ...
                        datos.setRP_APELLIDO(MM_RESULTADO.getString("APELLIDO"));
                        datos.setRP_CINRO(MM_RESULTADO.getString("CINRO"));
                        datos.setRP_TIPODOC(MM_RESULTADO.getString("TIPODOC"));
                        datos.setRP_IDCLINICA(MM_RESULTADO.getString("IDCLINICA"));
                        datos.setRP_IDCATEGORIA_PERSONA(MM_RESULTADO.getString("IDCATEGORIA_PERSONA"));
                        datos.setRP_DESC_CATEG_PERSONA(MM_RESULTADO.getString("DESC_CATEG_PERSONA"));
                        datos.setRP_RAZON_SOCIAL(MM_RESULTADO.getString("RAZON_SOCIAL"));
                        datos.setRP_RUC(MM_RESULTADO.getString("RUC"));
                        datos.setRP_DIRECCION(MM_RESULTADO.getString("DIRECCION"));
                        datos.setRP_EMAIL(MM_RESULTADO.getString("EMAIL"));
                        datos.setRP_TELFPARTICULAR(MM_RESULTADO.getString("TELFPARTICULAR"));
                        datos.setRP_TELFMOVIL(MM_RESULTADO.getString("TELFMOVIL"));
                        datos.setRP_TELEFPARTICULAR(MM_RESULTADO.getString("TELEFPARTICULAR"));
...
//                System.out.println(".");
        ...
        sesionDatos.setAttribute("PAG_MED_LISTA_ACTUAL", "1");
        // CARGO EL TOTAL DE LISTAS
        sesionDatos.setAttribute("PAG_MED_CANT_TOTAL_BTNS", ""+cant_btn_lista_final+"");
//        sesionDatos.setAttribute("PAG_MED_CANT_TOTAL_BTNS", ""+cant_btn_lista+"");
...
//        PARAM_SESION.setAttribute("PAG_PC_ANU_FACT_CANT_TOTAL_ROWS", ""+cant_resultado+"");
        ...
        sesionDatos.setAttribute("PAG_MED_CANT_ROWS_MOSTRAR", ""+PARAM_NRO_REG_MOSTRAR+"");
        // VARIABLE QUE UTILIZO PARA SABER LOS NUMEROS DE LOS BOTONES A MOSTRAR DEACUERDO A LA CANTIDAD DE CLIS DERECHOS 
        sesionDatos.setAttribute("PAG_MED_CANT_BTN_DERE_CLIC", ""+CANT_CLICS_DERECHO+"");
...
//                "AND rp.IDPERSONA NOT IN (0,1) \n" +
//                "AND IDCATEGORIA_PERSONA = "+IDCATEG_PER_MEDICO+" \n";
//            System.out.println("------------------------MODEL_PERSONA--------------------------");
...
//                    }
//                    datos.setRP_RUC(RUC);
//                    datos.setRP_IDBARRIO(MM_RESULTADO.getString("IDBARRIO"));
//                    datos.setRP_DESC_BARRIO(MM_RESULTADO.getString("DESC_BARRIO"));
//                    datos.setRP_IDCIUDAD(MM_RESULTADO.getString("IDCIUDAD"));
//                    datos.setRP_DESC_CIUDAD(MM_RESULTADO.getString("DESC_CIUDAD"));
//                    if (MM_RESULTADO.getString("DIRECCION") != null) {
//                        DIRECCION = MM_RESULTADO.getString("DIRECCION");
//                    } else {
//                        DIRECCION = "";
...
//                    }
//                    datos.setRP_TELFMOVIL(TELFMOVIL);
//                    datos.setRP_IDCIUDADNAC(MM_RESULTADO.getString("IDCIUDADNAC"));
//                    datos.setRP_DESC_CIUDADNAC(MM_RESULTADO.getString("DESC_CIUDADNAC"));
//                    datos.setRP_FECHANAC(MM_RESULTADO.getString("FECHANAC"));
//                    if (MM_RESULTADO.getString("SEXO") != null) {
//                        SEXO = MM_RESULTADO.getString("SEXO");
//                    } else {
//                        SEXO = "";
//                    }
//                    datos.setRP_SEXO(SEXO);
//                    if (MM_RESULTADO.getString("RELIGION") != null) {
//                        RELIGION = MM_RESULTADO.getString("RELIGION");
//                    } else {
//                        RELIGION = "";
//                    }
//                    datos.setRP_RELIGION(RELIGION);
//                    if (MM_RESULTADO.getString("ESTADOCIVIL") != null) {
//                        ESTADOCIVIL = MM_RESULTADO.getString("ESTADOCIVIL");
//                    } else {
//                        ESTADOCIVIL = "";
//                    }
//                    datos.setRP_ESTADOCIVIL(ESTADOCIVIL);
//                    if (MM_RESULTADO.getString("GRUPOSANGUINEO") != null) {
//                        GRUPOSANGUINEO = MM_RESULTADO.getString("GRUPOSANGUINEO");
//                    } else {
//                        GRUPOSANGUINEO = "";
//                    }
//                    datos.setRP_GRUPOSANGUINEO(GRUPOSANGUINEO);
//                    if (MM_RESULTADO.getString("OBSERVACION") != null) {
//                        OBSERVACION = MM_RESULTADO.getString("OBSERVACION");
//                    } else {
//                        OBSERVACION = "";
//                    }
//                    datos.setRP_OBSERVACION(OBSERVACION);
//                    datos.setRP_FECHAALTA("FECHAALTA");
//                    if (MM_RESULTADO.getString("FECULTIMOMOV") != null) {
//                        FECULTIMOMOV = MM_RESULTADO.getString("FECULTIMOMOV");
//                    } else {
//                        FECULTIMOMOV = "";
//                    }
//                    datos.setRP_FECULTIMOMOV(FECULTIMOMOV);
//                    if (MM_RESULTADO.getString("FOTO") != null) {
//                        FOTO = MM_RESULTADO.getString("FOTO");
//                    } else {
//                        FOTO = "";
//                    }
//                    datos.setRP_FOTO(FOTO);
//                    datos.setRP_USU_ALTA(MM_RESULTADO.getString("USU_ALTA"));
//                    if (MM_RESULTADO.getString("USU_MOD") != null) {
//                        USU_MOD = MM_RESULTADO.getString("USU_MOD");
//                    } else {
//                        USU_MOD = "";
//                    }
//                    datos.setRP_USU_MOD(USU_MOD);
//                    datos.setRP_IDPAIS(MM_RESULTADO.getString("IDPAIS"));
//                    datos.setRP_DESC_PAIS(MM_RESULTADO.getString("DESC_PAIS"));
//                    if (MM_RESULTADO.getString("TELEFPARTICULAR") != null) {
//                        TELEFPARTICULAR = MM_RESULTADO.getString("TELEFPARTICULAR");
//                    } else {
//                        TELEFPARTICULAR = "";
...
//        ModelUsuario metodoUsuario = new ModelUsuario();
        ...
        try {
            int varOperacion;
            String sql;
            String IDPERSONA = medico.getRP_IDPERSONA();
            String NOMBRE = medico.getRP_NOMBRE();
            String APELLIDO = medico.getRP_APELLIDO();
            String CINRO = medico.getRP_CINRO();
            String TIPODOC = medico.getRP_TIPODOC();
            String IDCATEGORIA_PERSONA = medico.getRP_IDCATEGORIA_PERSONA();
            String DESC_CATEG_PERSONA = medico.getRP_DESC_CATEG_PERSONA();
            String RAZON_SOCIAL = medico.getRP_RAZON_SOCIAL();
            String RUC = medico.getRP_RUC();
            String DIRECCION = medico.getRP_DIRECCION();
            String EMAIL = medico.getRP_EMAIL();
            String IDBARRIO = medico.getRP_IDBARRIO();
//            String DESC_BARRIO = medico.getRP_DESC_BARRIO();
            String IDCIUDAD = medico.getRP_IDCIUDAD();
//            String DESC_CIUDAD = medico.getRP_DESC_CIUDAD();
            String TELFPARTICULAR = medico.getRP_TELFPARTICULAR();
            String TELFMOVIL = medico.getRP_TELFMOVIL();
            String IDCIUDADNAC = medico.getRP_IDCIUDADNAC();
//            String DESC_CIUDADNAC = medico.getRP_DESC_CIUDADNAC();
            String FECHANAC = medico.getRP_FECHANAC();
            String SEXO = medico.getRP_SEXO();
            String RELIGION = medico.getRP_RELIGION();
            String ESTADOCIVIL = medico.getRP_ESTADOCIVIL();
            String GRUPOSANGUINEO = medico.getRP_GRUPOSANGUINEO();
            String OBSERVACION = medico.getRP_OBSERVACION();
            String FECHAALTA = medico.getRP_FECHAALTA();
            String FECULTMOMOV = medico.getRP_FECULTIMOMOV();
            String FOTO = medico.getRP_FOTO();
            String USU_ALTA = medico.getRP_USU_ALTA();
            String USU_MOD = medico.getRP_USU_MOD();
            String IDPAIS = medico.getRP_IDPAIS();
//            String DESC_PAIS = medico.getRP_DESC_PAIS();
            String TELEFPARTICULAR = medico.getRP_TELEFPARTICULAR();
            String OCUPACION = medico.getRP_OCUPACION();
            String ANTECEDENTES = medico.getRP_ANTECEDENTES();
            String EXPEDIENTE_CLINICO = medico.getRP_EXPEDIENTE_CLINICO();
            String IDCLINICA = medico.getRP_IDCLINICA();
            String SEGURO_MEDICO = medico.getRP_SEGURO_MEDICO();
            // USUARIO 
//            String SU_IDUSUARIO = medico.getSU_IDUSUARIO();
            String SU_IDPERSONA = IDPERSONA;
//            String SU_USUARIO = medico.getSU_USUARIO();
//            String SU_CLAVE = medico.getSU_CLAVE();
//            String SU_ESTADO = medico.getSU_ESTADO();
//            String SU_IDPERFIL = medico.getSU_IDPERFIL();
//            String SU_DESC_PERFIL = medico.getSU_DESC_PERFIL();
//            String SU_ESTADO_MIGRAR = medico.getSU_ESTADO_MIGRAR();
//            String SU_WEB = medico.getSU_WEB();
//            String SU_EMAIL = medico.getSU_EMAIL();
            
            // CONTROLO POR MEDIO DEL IDPERSONA DEL MEDICO QUE OPERACION DEBERIA DE REALIZAR, SI LA DE UPDATE O LA DE INSERT 
            if (medico.getRP_IDPERSONA() == null || medico.getRP_IDPERSONA().isEmpty() || medico.getRP_IDPERSONA().equals("")) {
            ...
            MM_CONEXION = devolverConex();
            MM_SENTENCIA = MM_CONEXION.createStatement();
            int cantResul = MM_SENTENCIA.executeUpdate(sql);
...
//                respuesta = "No se ha podido guardar, vuelva a intentarlo.";
    ...
    // METODO PARA REALIZAR LA PAGINACION Y FILTRO SOBRE LA TABLA DE PERSONA SOBRE EL IDCATEGORIA PERSONA DEL MEDICO 
...
//        }
        ...
        if (sesionDatos.getAttribute("PAG_MED_LISTA_ACTUAL") == null || String.valueOf(sesionDatos.getAttribute("PAG_MED_LISTA_ACTUAL")).isEmpty()) {
            NRO_PAG_ACTUAL_MOSTRAR = "1";
        } else {
            NRO_PAG_ACTUAL_MOSTRAR = (String) sesionDatos.getAttribute("PAG_MED_LISTA_ACTUAL");
        }
        ...
        String cant_min_fija = PARAM_CBX_MOSTRAR;
         String cant_inicial_anterior = (String) sesionDatos.getAttribute("PAG_MED_CANT_ROWS_MOSTRAR"); // ESTA ES LA CANTIDAD DE LA ANTERIOR SELECCION DE CANTIDAD DE FILAS A MOSTRAR EN LA GRILLA, UTILIZO ESTA VARIABLE PARA COMPARARLA CON LA DE AHORA PARA SABER SI QUIERE CAMBIAR LA CANTIDAD DE FILAS A MOSTRAR O QUIERE CAMBIAR DE PAGINA NOMAS O ACTUALIZAR EL FILTRO 
...
//                System.out.println(".");
        ...
        // CARGO LA PAGINA ACTUAL A MOSTRAR 
        sesionDatos.setAttribute("PAG_MED_LISTA_ACTUAL", NRO_PAG_ACTUAL_MOSTRAR);
        // CARGO EL TOTAL DE LISTAS
        sesionDatos.setAttribute("PAG_MED_CANT_TOTAL_BTNS", ""+cant_btn_lista_final+"");
//        sesionDatos.setAttribute("PAG_MED_CANT_TOTAL_BTNS", ""+cant_btn_lista+"");
        // VARIABLE QUE UTILIZO PARA SABER LA CANTIDAD DE FILAS QUE SE MUESTRAN PARA LUEGO UTILIZARLO AL COMIENZO DEL METODO PARA HACER UNA VALIDACION 
        sesionDatos.setAttribute("PAG_MED_CANT_ROWS_MOSTRAR", ""+PARAM_CBX_MOSTRAR+"");
    ...
    */
    public Map<String, String> cargarComboMedicos(String PARAM_IDMEDICO, HttpSession sesionDatos) { // PARAM_IDMEDICO : PARAMETRO QUE UTILIZO PARA NO FILTRAR POR EL MEDICO OCASIONAL PASANDOLE SU ID, EN ALGUNAS PAGINAS COMO EMPEÑO Y PASANDOLE VACIO PARA MOSTRAR COMO EN FACTURA 
        ...
        try {
            String sql = "SELECT rp.IDPERSONA, rp.NOMBRE, rp.APELLIDO, rp.CINRO, rp.TIPODOC, rp.IDCATEGORIA_PERSONA, rp.DESC_CATEG_PERSONA, \n" +
                ...
                "WHERE rp.IDCLINICA = '"+VAR_ID_CLINICA+"' \n" + 
                "AND rp.IDCATEGORIA_PERSONA = "+IDCATEG_PER_MEDICO+" \n" +
                "AND rp.IDPERSONA NOT IN ("+PARAM_IDMEDICO+"1) \n" + /* IDPERSONA 1 ES ROOT */
//                "AND rp.IDPERSONA NOT IN (0,1) \n" + /* IDPERSONA 1 ES ROOT */
            ...
            while(MM_RESULTADO.next()) {
                String idPersona = MM_RESULTADO.getString("IDPERSONA");
                String cbxCLIENTE = MM_RESULTADO.getString("CINRO")+" - "+MM_RESULTADO.getString("APELLIDO")+" "+MM_RESULTADO.getString("NOMBRE");
    ...
    /*
        METODO QUE UTILIZO PARA CARGAR COMBOBOX CON LOS MEDICOS, UTILIZO MAS PARA LOS BUSCADORES 
        MEDICOS CON PLAN HORARIO EN LA IDCLINICA LOGEADA 
    */
    public Map<String, String> traerComboMedicosPH(String PARAM_IDMEDICO, HttpSession PARAM_SESION) {
        Map<String, String> mapaList = new LinkedHashMap<>();
        ModelPersona metodosPersona = new ModelPersona();
        ModelPerfil metodosPerfil = new ModelPerfil();
        try {
            // RECUPERO ACA EL PERFIL Y NO MAS ADELANTE PARA TENER LAS CONEXIONES CERRANDOSE EN ORDEN Y NO UNO DENTRO DE OTRO 
            String IDPERFIL_USER = (String) PARAM_SESION.getAttribute("IDPERFIL");
            System.out.println(".   _model____ID_PERFIL_USER:        :"+IDPERFIL_USER);
            int MOSTRAR_CINRO;
            // CONTROLO EL PERFIL PARA SABER QUE PERFIL TIENE EL USUARIO LOGEADO Y DEACUERDO A ESO MOSTRAR EL NRO DE CI DEL MEDICO O NO 
            if (metodosPerfil.isPerfilPaciente(IDPERFIL_USER)) {
                MOSTRAR_CINRO = 1; // SI FUERA EL PERFIL PACIENTE, ENTONCES NO LE VOY A MOSTRAR EL NRO DE CI COMO DATO DEL MEDICO Y SOLO LE MUESTRO EL NOMBRE Y EL APELLIDO 
            } else /*if (metodosPerfil.isPerfilAdmin(IDPERFIL_USER) || metodosPerfil.isPerfilSecre(IDPERFIL_USER))*/ {
                MOSTRAR_CINRO = 0; // SI FUERA EL PERFIL ADMINISTRADOR O SECRETARIO, ENTONCES LE MUESTRO EL NRO DE CI DEL MEDICO PARA AGREGAR MAS INFO AL SECRETARIO O ADMINISTRADOR 
            }
            System.out.println(".   ____MOSTRAR_CINRO_MED:        :"+MOSTRAR_CINRO);
            
            String whereParamMedico = "AND rp.IDPERSONA NOT IN (0, 1"; // VARIABLE QUE UTILIZARE PARA QUE EN CASO DE QUE EL COMBO DEBA SER CARGADO POR EL MEDICO QUE LE PASO POR PARAMETRO, EN EL SELECT NO VUELVA A TRAER ESE MEDICO PARA QUE NO SE REPITA EN EL COMBO 
//            String whereParamMedico = ""; // VARIABLE QUE UTILIZARE PARA QUE EN CASO DE QUE EL COMBO DEBA SER CARGADO POR EL MEDICO QUE LE PASO POR PARAMETRO, EN EL SELECT NO VUELVA A TRAER ESE MEDICO PARA QUE NO SE REPITA EN EL COMBO 
            ...
            if (!(PARAM_IDMEDICO.equals("")) || !(PARAM_IDMEDICO.isEmpty())) {
                List<BeanPersona> listMedico = new ArrayList<>();
                listMedico = metodosPersona.traerDatosPersona(PARAM_IDMEDICO);
                ...
                while(iterParamMed.hasNext()) {
                    ...
                    // AL IGUAL QUE EN EL WHILE CONTROLO LA BANDERA PARA SABER QUE DATOS MOSTRARLE 
                    if (MOSTRAR_CINRO == 1) {
                        MEDICO_NAME = datosMedico.getRP_NOMBRE()+" "+datosMedico.getRP_APELLIDO();
                    } else {
                        MEDICO_NAME = datosMedico.getRP_CINRO()+" - "+datosMedico.getRP_NOMBRE()+" "+datosMedico.getRP_APELLIDO();
                    }
                }
                mapaList.put(PARAM_IDMEDICO, MEDICO_NAME);
                whereParamMedico = whereParamMedico+", "+PARAM_IDMEDICO+")";
//                whereParamMedico = "AND rp.IDPERSONA NOT IN ('"+PARAM_IDMEDICO+"')";
            } else {
                whereParamMedico = whereParamMedico+")";
            }
            
            VAR_ID_CLINICA = (String) PARAM_SESION.getAttribute("ID_CLINICA_USER"); // idclinica del usuario logeado 
            
            /*
             * OBSERVACION: SELECT QUE TRAE A LOS MEDICOS QUE TENGAN UN PLAN HORARIO DENTRO DE LA CLINICA DE LOGEO DEL USER 
            */
            String sql = "SELECT oph.IDPLANHORARIO, oph.IDCLINICA, rp.IDPERSONA, rp.NOMBRE, rp.APELLIDO, rp.CINRO, rp.IDCATEGORIA_PERSONA, rp.DESC_CATEG_PERSONA \n" +
                "FROM ope_plan_horario oph \n" +
                "JOIN rh_persona rp ON rp.IDPERSONA = oph.IDMEDICO AND rp.IDCATEGORIA_PERSONA = "+IDCATEG_PER_MEDICO+" \n" +
                "WHERE oph.ESTADO = 'A' \n" +
            ...
            /*
             * OBSERVACION: SELECT QUE TRAE A LOS MEDICOS DEACUERDO AL IDCLINICA DEL USER 
            */
//            String sql = "SELECT rp.IDPERSONA, rp.NOMBRE, rp.APELLIDO, rp.CINRO, rp.TIPODOC, rp.IDCATEGORIA_PERSONA, rp.DESC_CATEG_PERSONA, \n" +
//                "rp.RAZON_SOCIAL, rp.RUC, rp.DIRECCION, rp.EMAIL, rp.TELFPARTICULAR, rp.TELFMOVIL, rp.TELEFPARTICULAR, rp.IDCLINICA \n" +
//                "FROM rh_persona rp \n" +
...
//                "ORDER BY rp.CINRO ASC, rp.APELLIDO ASC, rp.NOMBRE ASC \n";
            ...
            while(MM_RESULTADO.next()) {
                String idPersona = MM_RESULTADO.getString("IDPERSONA");
                String cbxCLIENTE = MM_RESULTADO.getString("NOMBRE")+" "+MM_RESULTADO.getString("APELLIDO");
                if (MOSTRAR_CINRO == 0) { // SI CAMBIA SU VALOR A 1 ENTONCES NO LE AGREGO EL NRO DE CI DEL MEDICO COMO DATO VISIBLE YA QUE EL USUARIO ES UN PACIENTE 
                    cbxCLIENTE = MM_RESULTADO.getString("CINRO")+" - "+MM_RESULTADO.getString("NOMBRE")+" "+MM_RESULTADO.getString("APELLIDO");
                }
                mapaList.put(idPersona, cbxCLIENTE);
            }
            cerrarConexiones();
    ...
    /*
        METODO QUE UTILIZO PARA CARGAR COMBOBOX CON LOS MEDICOS, UTILIZO MAS PARA LOS BUSCADORES 
        MEDICOS DE LA IDCLINICA LOGEADA 
    */
    public Map<String, String> traerComboMedicos(String PARAM_IDMEDICO, HttpSession sesionDatos) {
        Map<String, String> mapaList = new LinkedHashMap<>();
        ModelPersona metodosPer = new ModelPersona();
        ModelPerfil metodosPerfil = new ModelPerfil();
        try {
            ...
            System.out.println(".   _model_MED____ID_PERFIL_USER:        :"+IDPERFIL_USER);
            int MOSTRAR_CINRO;
            // CONTROLO EL PERFIL PARA SABER QUE PERFIL TIENE EL USUARIO LOGEADO Y DEACUERDO A ESO MOSTRAR EL NRO DE CI DEL MEDICO O NO 
            if (metodosPerfil.isPerfilPaciente(IDPERFIL_USER)) {
                MOSTRAR_CINRO = 1; // SI FUERA EL PERFIL PACIENTE, ENTONCES NO LE VOY A MOSTRAR EL NRO DE CI COMO DATO DEL MEDICO Y SOLO LE MUESTRO EL NOMBRE Y EL APELLIDO 
...
//            String whereParamMedico = ""; // VARIABLE QUE UTILIZARE PARA QUE EN CASO DE QUE EL COMBO DEBA SER CARGADO POR EL MEDICO QUE LE PASO POR PARAMETRO, EN EL SELECT NO VUELVA A TRAER ESE MEDICO PARA QUE NO SE REPITA EN EL COMBO 
            ...
            if (!(PARAM_IDMEDICO.equals("")) || !(PARAM_IDMEDICO.isEmpty())) {
                List<BeanPersona> listMedico = new ArrayList<>();
                listMedico = metodosPer.traerDatosPersona(PARAM_IDMEDICO);
                Iterator<BeanPersona> iterParamMed = listMedico.iterator();
                BeanPersona datosMedico = new BeanPersona();
                String MEDICO_NAME = "";
                while(iterParamMed.hasNext()) {
