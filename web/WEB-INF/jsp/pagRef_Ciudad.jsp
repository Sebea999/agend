    ...
    <body>
        <%@include file="menu_jsp.jsp" %> <% // INCLUYO EL MENU %>
            <main>
                ...
                String mensaje = (String) request.getAttribute("CRCI_MENSAJE"); // CONTROLLER REFERENCIAL CIUDAD MENSAJE 
                String tipoMensaje = (String) request.getAttribute("CRCI_TIPO_MENSAJE"); // CONTROLLER REFERENCIAL CIUDAD TIPO MENSAJE 
                String claseMensaje = "";
                ...
                if (mensaje != null) { // SI ES DIFERENTE A NULL ENTONCES MOSTRARE LAS ETIQUETAS PARA QUE EL USUARIO PUEDA VER EL MENSAJE 
                    ...
                    if ((request.getAttribute("CRCI_MENSAJE_AUX") == null) == false) {
                        SEGUNDO_MENSAJE = (String) request.getAttribute("CRCI_MENSAJE_AUX");
...
//                        sesionDatosUsuario.removeAttribute("VALI_DELETE_PAC_NAME");
                ...
                <form action="CRCiSrv" method="post" autocomplete="off">
                    <div>
                        <h4 class="mainTitle">GESTION DE CIUDADES</h4>
                    </div>
                    <div>
                        <input type="submit" name="accion" value="Add Ciudad" class="btn btn-info mb-3" />
                    </div>
                    <div class="mainFilter">
                        <%
                        // RECUPERAMOS LOS DATOS DEL FILTRO EN CASO DE QUE SE HAGA PARA VOLVER A MOSTRAR Y SI NO, ENTONCES PASARIA DATOS VACIOS 
                        String cbxMostrar = (String) request.getAttribute("CRCI_CBX_MOSTRAR");
                        if (cbxMostrar == null || cbxMostrar.isEmpty()) {
                            cbxMostrar = "";
                        }
                        String txtFiltro = (String) request.getAttribute("CRCI_TXT_BUSCAR");
...
<%//                                <!--<a href="<%=urlRefClinica>" class="btn btn-primary mb-2">Limpiar</a>-->%>
                    ...
                    <div class="divTable">
                        <table role="table" class="tableFilter">
                            <thead role="rowgroup">
                                <tr role="row">
                                    <td role="columnheader" class="RCICI">Cód.</td>
                                    <td role="columnheader" class="RCICC">Ciudad</td>
                                    <td role="columnheader" class="RCICE">Estado</td>
                                    <td role="columnheader" class="RCICBA">Acciones</td>
                                </tr>
                            </thead>
                            <tbody role="rowgroup">
                                <%
                                    ...
                                    String NRO_PAG = "";
                                    if (sesionDatosUsuario.getAttribute("PAG_REFCIU_LISTA_ACTUAL") == null) {
                                        NRO_PAG = "1";
                                    } else {
                                        NRO_PAG = (String) sesionDatosUsuario.getAttribute("PAG_REFCIU_LISTA_ACTUAL");
                                    }
                                    System.out.println("_   _JSP____NRO_PAG_ACTUAL:     :"+NRO_PAG);
                                    
                                    List<BeanCiudad> listaDatos = null;
                                    if (((List<BeanCiudad>)request.getAttribute("CRCI_LISTA_FILTRO")) != null) { // si se hace el filtro entonces en la lista se debe almacenar del resultado que le pasamos 
                                        listaDatos = (List<BeanCiudad>) request.getAttribute("CRCI_LISTA_FILTRO");
                                        ...
                                        listaDatos = metodosRefCiudad.cargar_grilla_paginacion(sesionDatosUsuario, NRO_PAG, cant_min_fija);
                                        sesionDatosUsuario.setAttribute("CRCI_BAND_FILTRO", "0"); // CON ESTO RESETEO LA BANDERA QUE UTILIZO PARA LA PAGINACION 
                                    }
                                    Iterator<BeanCiudad> iterDatos = listaDatos.iterator();
                                    BeanCiudad datosBean = null;
                                    
                                    while(iterDatos.hasNext()) {
                                        datosBean = iterDatos.next();
                                %>
                                <tr role="row">
                                    <td role="cell" class="RCICI">
                                        <%= datosBean.getRC_IDCIUDAD() %>
                                    </td>
                                    <td role="cell" class="RCICC">
                                        <%= datosBean.getRC_DESC_CIUDAD()%>
                                    </td>
                                    <td  role="cell" class="RCICE">
                                        <%= metodosIniSes.devolverTextEstado(datosBean.getRC_ESTADO()) %>
                                    </td>
                                    <td role="cell" class="RCICBA">
                                        <form action="CRCiSrv" method="post">
                                            <input type="hidden" value="<%= datosBean.getRC_IDCIUDAD()%>" name="tI" class="form-control" />
                        ...
                        String cant_total_btn_lista = (String) sesionDatosUsuario.getAttribute("PAG_REFCIU_CANT_TOTAL_BTNS"); // PAGINA REFERENCIAL_CLINICA CANTIDAD TOTAL DE BOTONES 
                        ...
                        if ((String) sesionDatosUsuario.getAttribute("PAG_REFCIU_CANT_BTN_DERE_CLIC") == null) { // SI ESTA NULO POR ALGUNA RAZON DE INSTANCIA, ENTONCES LE COLOCO 1 COMO MINIMO VALOR ACEPTABLE Y SI NO ESTA NULO ENTONCES LE DEJO CON EL VALOR QUE TIENE 
                            CANT_CLIC_DERE = "1";
                        } else {
                            CANT_CLIC_DERE = (String) sesionDatosUsuario.getAttribute("PAG_REFCIU_CANT_BTN_DERE_CLIC");
                        }
