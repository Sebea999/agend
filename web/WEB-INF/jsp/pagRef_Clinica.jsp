    ...
    <body>
        <%@include file="menu_jsp.jsp" %> <% // INCLUYO EL MENU %>
            <main>
                ...
                String mensaje = (String) request.getAttribute("CRC_MENSAJE"); // CONTROLLER REFERENCIAL CLINICA MENSAJE 
                String tipoMensaje = (String) request.getAttribute("CRC_TIPO_MENSAJE"); // CONTROLLER REFERENCIAL CLINICA TIPO MENSAJE 
                String claseMensaje = "";
                ...
                if (mensaje != null) { // SI ES DIFERENTE A NULL ENTONCES MOSTRARE LAS ETIQUETAS PARA QUE EL USUARIO PUEDA VER EL MENSAJE 
                    ...
                    if ((request.getAttribute("CRC_MENSAJE_AUX") == null) == false) {
                        SEGUNDO_MENSAJE = (String) request.getAttribute("CRC_MENSAJE_AUX");
...
//                        sesionDatosUsuario.removeAttribute("VALI_DELETE_PAC_NAME");
                    }
                %>
                ...
                %>
                <form action="CRCSrv" method="post" autocomplete="off">
                    <div>
                        <h4 class="mainTitle">GESTION DE CLINICAS</h4>
                    </div>
                    <div>
                        <input type="submit" name="accion" value="Add Clinica" class="btn btn-info mb-3" />
                    </div>
                    <div class="mainFilter">
                        <%
                        // RECUPERAMOS LOS DATOS DEL FILTRO EN CASO DE QUE SE HAGA PARA VOLVER A MOSTRAR Y SI NO, ENTONCES PASARIA DATOS VACIOS 
                        String cbxMostrar = (String) request.getAttribute("CRC_CBX_MOSTRAR");
                        if (cbxMostrar == null || cbxMostrar.isEmpty()) {
                            cbxMostrar = "";
                        }
                        String txtFiltro = (String) request.getAttribute("CRC_TXT_BUSCAR");
...
<%//                                <!--<a href="<%=urlRefClinica>" class="btn btn-primary mb-2">Limpiar</a>-->%>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="divTable">
                        <table role="table" class="tableFilter">
                            <thead role="rowgroup">
                                <tr role="row">
                                    <td role="columnheader" class="RCCI">Cód.</td>
                                    <td role="columnheader" class="RCCC">Clinica</td>
                                    <td role="columnheader" class="RCCD">Dirección</td>
                                    <td role="columnheader" class="RCCE">Estado</td>
                                    <td role="columnheader" class="RCCBA">Acciones</td>
                                </tr>
                            </thead>
                            <tbody role="rowgroup">
                                <%
                                    ...
                                    String NRO_PAG = "";
                                    if (sesionDatosUsuario.getAttribute("PAG_REFCLI_LISTA_ACTUAL") == null) {
                                        NRO_PAG = "1";
                                    } else {
                                        NRO_PAG = (String) sesionDatosUsuario.getAttribute("PAG_REFCLI_LISTA_ACTUAL");
                                    }
                                    System.out.println("_   _JSP____NRO_PAG_ACTUAL:     :"+NRO_PAG);
                                    
                                    List<BeanClinica> listaDatos = null;
                                    if (((List<BeanClinica>)request.getAttribute("CRC_LISTA_FILTRO")) != null) { // si se hace el filtro entonces en la lista se debe almacenar del resultado que le pasamos 
                                        listaDatos = (List<BeanClinica>) request.getAttribute("CRC_LISTA_FILTRO");
                                        ...
                                        listaDatos = metodosClinica.cargar_grilla_paginacion(sesionDatosUsuario, NRO_PAG, cant_min_fija);
                                        sesionDatosUsuario.setAttribute("CRC_BAND_FILTRO", "0"); // CON ESTO RESETEO LA BANDERA QUE UTILIZO PARA LA PAGINACION 
                                    }
                                    Iterator<BeanClinica> iterDatos = listaDatos.iterator();
                                    BeanClinica datosBean = null;
                                    
                                    while(iterDatos.hasNext()) {
                                        datosBean = iterDatos.next();
                                %>
                                <tr role="row">
                                    <td role="cell" class="RCCI">
                                        <%= datosBean.getOC_IDCLINICA() %>
                                    </td>
                                    <td role="cell" class="RCCC">
                                        <%= datosBean.getOC_DESC_CLINICA()%>
                                    </td>
                                    <td role="cell" class="RCCD">
                                        <%= datosBean.getOC_DIRECCION() %>
                                    </td>
                                    <td  role="cell" class="RCCE">
                                        <%= metodosIniSes.devolverTextEstado(datosBean.getOC_ESTADO()) %>
                                    </td>
                                    <td role="cell" class="RCCBA">
                                        <form action="CRCSrv" method="post">
                                            <input type="text" value="<%= datosBean.getOC_IDCLINICA()%>" name="tI" class="form-control" />
                        ...
                        String cant_total_btn_lista = (String) sesionDatosUsuario.getAttribute("PAG_REFCLI_CANT_TOTAL_BTNS"); // PAGINA REFERENCIAL_CLINICA CANTIDAD TOTAL DE BOTONES 
                        ...
                        if ((String) sesionDatosUsuario.getAttribute("PAG_REFCLI_CANT_BTN_DERE_CLIC") == null) { // SI ESTA NULO POR ALGUNA RAZON DE INSTANCIA, ENTONCES LE COLOCO 1 COMO MINIMO VALOR ACEPTABLE Y SI NO ESTA NULO ENTONCES LE DEJO CON EL VALOR QUE TIENE 
                            CANT_CLIC_DERE = "1";
                        } else {
                            CANT_CLIC_DERE = (String) sesionDatosUsuario.getAttribute("PAG_REFCLI_CANT_BTN_DERE_CLIC");
                        }
