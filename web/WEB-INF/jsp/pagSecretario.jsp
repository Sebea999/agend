<%-- 
    Document   : pagSecretario
    Created on : 11-dic-2021, 11:14:23
    Author     : RYUU
--%>
        ...
        <%@include file="menu_jsp.jsp" %> <% // INCLUYO EL MENU %>
            ...
            %>
                <form action="CSSrv" method="post" autocomplete="off">
                    <div>
                        <h4 class="mainTitle">SECRETARIO - GESTION DE SECRETARIOS</h4>
                    </div>
                    <div>
                        <input type="submit" name="accion" value="Add Secretario" class="btn btn-outline-danger mb-2 btnAddCli" />
                    </div>
                    <div class="mainFilter">
                        <%
                        // RECUPERAMOS LOS DATOS DEL FILTRO EN CASO DE QUE SE HAGA PARA VOLVER A MOSTRAR Y SI NO, ENTONCES PASARIA DATOS VACIOS 
                        String cbxMostrar = (String) request.getAttribute("CS_CBX_MOSTRAR");
                        if (cbxMostrar == null || cbxMostrar.isEmpty()) {
                            cbxMostrar = "";
                        }
                        String txtFiltro = (String) request.getAttribute("CS_TXT_BUSCAR");
...
<%//                                <!--<a href="<%=urlSecretario>" class="btn btn-primary mb-2">Limpiar</a>-->%>
                            </div>
                        </div>
                    </div>
                        ...
                        <table role="table" class="tableFilter">
                            <thead role="rowgroup">
                                <tr role="row">
                                    ...
                                    <%--<td role="columnheader" class="P_CCI">Clinica</td>--%>
                                    ...
                                    <td role="columnheader" class="P_CE">Acción</td>
                                </tr>
                            </thead>
                            <tbody role="rowgroup">
                                <%
                                    ...
                                    String NRO_PAG = "";
                                    if (sesionDatosUsuario.getAttribute("PAG_SEC_LISTA_ACTUAL") == null) {
                                        NRO_PAG = "1";
                                    } else {
                                        NRO_PAG = (String) sesionDatosUsuario.getAttribute("PAG_SEC_LISTA_ACTUAL");
                                    }
                                    System.out.println("_   _JSP____NRO_PAG_ACTUAL:     :"+NRO_PAG);
                                    
                                    List<BeanPersona> listaDatos = null;
                                    if (((List<BeanPersona>)request.getAttribute("CS_LISTA_FILTRO")) != null) { // si se hace el filtro entonces en la lista se debe almacenar del resultado que le pasamos 
                                        listaDatos = (List<BeanPersona>) request.getAttribute("CS_LISTA_FILTRO");
                                    } else {
                                        ...
                                        listaDatos = metodosSecretario.cargar_grilla_paginacion(sesionDatosUsuario, NRO_PAG, cant_min_fija);
                                        sesionDatosUsuario.setAttribute("CS_BAND_FILTRO", "0"); // CON ESTO RESETEO LA BANDERA QUE UTILIZO PARA LA PAGINACION 
                                    }
...
//                                        String IDCLINICA = datosBean.getRP_IDCLINICA();
...
<%--                                    <td role="cell" class="P_CCI">
                        ...
                        String cant_total_btn_lista = (String) sesionDatosUsuario.getAttribute("PAG_SEC_CANT_TOTAL_BTNS"); // PAGINA SECRETARIO CANTIDAD TOTAL DE BOTONES 
//                        System.out.println("_   __JSP___TOTAL_BTN_LISTA:   :"+cant_total_btn_lista);
                        ...
                        if ((String) sesionDatosUsuario.getAttribute("PAG_SEC_CANT_BTN_DERE_CLIC") == null) { // SI ESTA NULO POR ALGUNA RAZON DE INSTANCIA, ENTONCES LE COLOCO 1 COMO MINIMO VALOR ACEPTABLE Y SI NO ESTA NULO ENTONCES LE DEJO CON EL VALOR QUE TIENE 
                            CANT_CLIC_DERE = "1";
                        } else {
                            CANT_CLIC_DERE = (String) sesionDatosUsuario.getAttribute("PAG_SEC_CANT_BTN_DERE_CLIC");
                        }
