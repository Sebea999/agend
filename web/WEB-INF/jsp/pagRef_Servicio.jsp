<%-- 
    Document   : pagRef_Servicio
    Created on : 22-feb-2022, 10:21:21
    Author     : RYUU
--%>
    ...
    <body>
        <%@include file="menu_jsp.jsp" %> <% // INCLUYO EL MENU %>
            <main>
                ...
                String mensaje = (String) request.getAttribute("CRS_MENSAJE"); // CONTROLLER REFERENCIAL SERVICIO MENSAJE 
                String tipoMensaje = (String) request.getAttribute("CRS_TIPO_MENSAJE"); // CONTROLLER REFERENCIAL SERVICIO TIPO MENSAJE 
                String claseMensaje = "";
                ...
                <form action="CRSSrv" method="post" autocomplete="off">
                    <div>
                        <h4 class="mainTitle">GESTION DE SERVICIOS</h4>
                    </div>
                    <div>
                        <input type="submit" name="accion" value="Add Servicio" class="btn btn-info mb-3" />
                    </div>
                    <div class="mainFilter">
                        <%
                        // RECUPERAMOS LOS DATOS DEL FILTRO EN CASO DE QUE SE HAGA PARA VOLVER A MOSTRAR Y SI NO, ENTONCES PASARIA DATOS VACIOS 
                        String cbxMostrar = (String) request.getAttribute("CRS_CBX_MOSTRAR");
                        if (cbxMostrar == null || cbxMostrar.isEmpty()) {
                            cbxMostrar = "";
                        }
                        String txtFiltro = (String) request.getAttribute("CRS_TXT_BUSCAR");
...
<%//                                <!--<a href="<%= // urlRefServicios >" class="btn btn-primary mb-2">Limpiar</a>-->%>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="divTable">
                        <table role="table" class="tableFilter">
                            <thead role="rowgroup">
                                <tr role="row">
                                    ...
                                    <td role="columnheader" class="RECD">Servicio</td>
                                    <td role="columnheader" class="RECI">IVA</td>
                            ...
                            <tbody role="rowgroup">
                                <%
                                    ...
                                    String NRO_PAG = "";
                                    if (sesionDatosUsuario.getAttribute("PAG_REFSERV_LISTA_ACTUAL") == null) {
                                        NRO_PAG = "1";
                                    } else {
                                        NRO_PAG = (String) sesionDatosUsuario.getAttribute("PAG_REFSERV_LISTA_ACTUAL");
                                    }
                                    System.out.println("_   _JSP____NRO_PAG_ACTUAL:     :"+NRO_PAG);
                                    
                                    List<BeanServicio> listaDatos = null;
                                    if (((List<BeanServicio>)request.getAttribute("CRS_LISTA_FILTRO")) != null) { // si se hace el filtro entonces en la lista se debe almacenar del resultado que le pasamos 
                                        listaDatos = (List<BeanServicio>) request.getAttribute("CRS_LISTA_FILTRO");
                                        ...
                                        listaDatos = metodosRefServicio.cargar_grilla_paginacion(sesionDatosUsuario, NRO_PAG, cant_min_fija);
                                        sesionDatosUsuario.setAttribute("CRS_BAND_FILTRO", "0"); // CON ESTO RESETEO LA BANDERA QUE UTILIZO PARA LA PAGINACION 
                                    }
                                    Iterator<BeanServicio> iterDatos = listaDatos.iterator();
                                    BeanServicio datosBean = null;
                                    
                                    while(iterDatos.hasNext()) {
                                        datosBean = iterDatos.next();
                                        
                                        String IVA = datosBean.getRHS_IVA();
                                ...
                                <tr role="row">
                                    <td role="cell" class="RECI">
                                        <%= datosBean.getRHS_IDSERVICIO()%>
                                    </td>
                                    <td role="cell" class="RECD">
                                        <%= datosBean.getRHS_DESC_SERVICIO()%>
                                    </td>
                                    <td role="cell" class="RECI">
                                        <%= IVA%>
                                    </td>
                                    <td  role="cell" class="RECE">
                                        <%= formatear.format(Double.parseDouble(datosBean.getRHS_MONTO())) %>
                                    </td>
                                    <td  role="cell" class="RECE">
                                        <%= metodosIniSes.devolverTextEstado(datosBean.getRHS_ESTADO()) %>
                                    </td>
                                    <td role="cell" class="RECBA">
                                        <form action="CRSSrv" method="post">
                                            <input type="text" value="<%= datosBean.getRHS_IDSERVICIO()%>" name="tI" class="form-control" />
                        ...
                        String cant_total_btn_lista = (String) sesionDatosUsuario.getAttribute("PAG_REFSERV_CANT_TOTAL_BTNS"); // PAGINA REFERENCIAL_CLINICA CANTIDAD TOTAL DE BOTONES 
//                        System.out.println("_   __JSP___TOTAL_BTN_LISTA:   :"+cant_total_btn_lista);
                        ...
                        if ((String) sesionDatosUsuario.getAttribute("PAG_REFSERV_CANT_BTN_DERE_CLIC") == null) { // SI ESTA NULO POR ALGUNA RAZON DE INSTANCIA, ENTONCES LE COLOCO 1 COMO MINIMO VALOR ACEPTABLE Y SI NO ESTA NULO ENTONCES LE DEJO CON EL VALOR QUE TIENE 
                            CANT_CLIC_DERE = "1";
                        } else {
                            CANT_CLIC_DERE = (String) sesionDatosUsuario.getAttribute("PAG_REFSERV_CANT_BTN_DERE_CLIC");
                        }
