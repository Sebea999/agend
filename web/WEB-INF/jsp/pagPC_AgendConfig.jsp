<%-- 
    Document   : pagPC_AgendConfig
    Created on : 19-jul-2022, 10:14:07
    Author     : RYUU
--%>
    ...
    <body>
        <%@include file="menu_jsp.jsp" %> <% // INCLUYO EL MENU %>
            ...
            <%
                ...
                String mensaje = (String) request.getAttribute("CPC_AC_MENSAJE"); // CONTROLLER PANEL CONTROL - AGENDAMIENTO CONFIG - MENSAJE 
            ...
            %>
                <form action="CPCSrv" method="post" autocomplete="off">
                    <div>
                        <h4 class="mainTitle">Configuración de Agendamiento</h4>
                    </div>
                    <div>
                        <input type="submit" name="accionAC" value="Add Nueva Configuración" class="btn btn-outline-danger mb-2 btnAddCli mt-2" />
                    </div>
                    <div class="mainFilter">
                        <%
                        // RECUPERAMOS LOS DATOS DEL FILTRO EN CASO DE QUE SE HAGA PARA VOLVER A MOSTRAR Y SI NO, ENTONCES PASARIA DATOS VACIOS 
                        String cbxMostrar = (String) request.getAttribute("CPC_AC_CBX_MOSTRAR");
                        if (cbxMostrar == null || cbxMostrar.isEmpty()) {
                            cbxMostrar = "";
                        }
                        String txtFiltro = (String) request.getAttribute("CPC_AC_TXT_BUSCAR");
                        if (txtFiltro == null || txtFiltro.isEmpty()) {
                            txtFiltro = "";
                        }
                        String txtFiltroEstado = (String) request.getAttribute("CPC_AC_TXT_FIL_ESTADO");
                        ...
                        <div class="mainFilterBox">
                            <p class="mainLabelSearch">Estado:</p>
                                ...
                                */
                                    String CHECK_FILTER_ESTADO = (String) request.getAttribute("CPC_AC_CHECK_FILTRAR_ESTADO"); // VARIABLE QUE SE ENCUENTRA EN EL MAV / REQUEST_MAPPING DE LA PAGINA 
                                    String CHECK_FILTER_ESTADO01 = (String) request.getAttribute("CPC_AC_CHECK_FILTRAR_ESTADO_01"); // VARIABLE QUE SE ENCUENTRA DENTRO DEL DO_POST 
                            ...
                            <div class="combomedico">
                                <select id="cE" name="cE" class="cbxEstado form-control">
                                <%
                                Map<String, String> listaEstado;
                                listaEstado = metodosIniSes.cargarComboEstado(txtFiltroEstado);
                        ...
                        <div class="mainFilterBox boxFiltroBtn">
                            ...
                            <div>
                                <input type="submit" name="accionAC" value="Filtrar" id="Filtrar" class="mb-2 mr-2 btn btn-primary" />
                                <button type="submit" name="accionAC" value="LimpiarPag" id="Limpiar" class="mb-2 btn btn-primary">Limpiar</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="divTable">
                        <table role="table" class="tableFilter">
                            <thead role="rowgroup">
                                <tr role="row">
                                    <td role="columnheader" class="PCAC_CI">Cód.</td>
                                    <td role="columnheader" class="PCAC_CDC">Descripción de la configuración</td>
                                    <td role="columnheader" class="PCAC_CC">Clinica</td>
                                    <td role="columnheader" class="PCAC_CE">Estado</td>
                                    <td role="columnheader" class="PCAC_CBA">Acciones</td>
                                </tr>
                            </thead>
                            <tbody role="rowgroup">
                                <%
                                    ...
                                    String NRO_PAG = "";
                                    if (sesionDatosUsuario.getAttribute("PAG_PC_AGEND_CONFIG_LISTA_ACTUAL") == null) {
                                        NRO_PAG = "1";
                                    } else {
                                        NRO_PAG = (String) sesionDatosUsuario.getAttribute("PAG_PC_AGEND_CONFIG_LISTA_ACTUAL");
                                    }
                                    System.out.println("_   _JSP____NRO_PAG_ACTUAL:     :"+NRO_PAG);
                                    
                                    List<BeanConfigAgend> listaDatos = null;
                                    if (((List<BeanConfigAgend>)request.getAttribute("CPC_AC_LISTA_FILTRO")) != null) { // si se hace el filtro entonces en la lista se debe almacenar del resultado que le pasamos 
                                        listaDatos = (List<BeanConfigAgend>) request.getAttribute("CPC_AC_LISTA_FILTRO");
                                    } else {
                                        ...
                                        listaDatos = metodosConfigAgend.cargar_grilla_paginacion(sesionDatosUsuario, NRO_PAG, cant_min_fija);
                                        sesionDatosUsuario.setAttribute("CPC_VAR_BAND_FILTRO", "0"); // CON ESTO RESETEO LA BANDERA QUE UTILIZO PARA LA PAGINACION 
                                    }
                                    Iterator<BeanConfigAgend> iterDatos = listaDatos.iterator();
                                    BeanConfigAgend datosBean = null;
                                    
                                    while(iterDatos.hasNext()) {
                                        datosBean = iterDatos.next();
                                        
                                        String IDCLINICA = datosBean.getSCA_IDCLINICA();
                                        String DESC_CLINICA = metodosRefClinica.traerDescClinica(IDCLINICA);
                                %>
                                <tr role="row">
                                    <td role="cell" class="PCAC_CI">
                                        <%= datosBean.getSCA_IDCONFIGAGEND() %>
                                    </td>
                                    <td role="cell" class="PCAC_CDC">
                                        <%= datosBean.getSCA_DESC_CONFIG()%>
                                    </td>
                                    <td role="cell" class="PCAC_CC">
                                        <%= DESC_CLINICA%>
                                    </td>
                                    <td role="cell" class="PCAC_CE">
                                        <%= metodosIniSes.devolverTextEstado(datosBean.getSCA_ESTADO()) %>
                                    </td>
                                    <td role="cell" class="PCAC_CBA">
                                        <form action="CPCSrv" method="post">
                                            <input type="text" value="<%= datosBean.getSCA_IDCONFIGAGEND() %>" name="tI" class="form-control" />
                                            <input type="submit" value="Editar" name="accionAC" class="btn btn-light" />
                                        </form>
                                    </td>
                                </tr>
                                <%
...
//                        System.out.println(".");
                        ...
                        String cant_total_btn_lista = (String) sesionDatosUsuario.getAttribute("PAG_PC_AGEND_CONFIG_CANT_TOTAL_BTNS"); // PAGINA PACIENTE CANTIDAD TOTAL DE BOTONES 
//                        System.out.println("_   __JSP___TOTAL_BTN_LISTA:   :"+cant_total_btn_lista);
                        ...
                        if ((String) sesionDatosUsuario.getAttribute("PAG_PC_AGEND_CONFIG_CANT_BTN_DERE_CLIC") == null) { // SI ESTA NULO POR ALGUNA RAZON DE INSTANCIA, ENTONCES LE COLOCO 1 COMO MINIMO VALOR ACEPTABLE Y SI NO ESTA NULO ENTONCES LE DEJO CON EL VALOR QUE TIENE 
                            CANT_CLIC_DERE = "1";
                        } else {
                            CANT_CLIC_DERE = (String) sesionDatosUsuario.getAttribute("PAG_PC_AGEND_CONFIG_CANT_BTN_DERE_CLIC");
                        }
...
//                        System.out.println("_   _JSP__btn_min_mostrar:  :"+btn_min_mostrar);
                        ...
                        */
                        ...
                        %>
                            <input type="submit" value="<<" name="accionAC" class="py-1 px-3 mr-2 btn btn-outline-secondary" />
                        <%
                        }
...
//                            System.out.println("__  _JPS_____if( i("+i+") <= resul_multi_primo("+btn_max_mostrar+") && i("+i+") >= pag_min_mostrar("+btn_min_mostrar+")   )____");
                            ...
                            %>
                            <input type="submit" value="<%= i %>" name="accionAC" class="py-1 px-3 mr-2 <%=btn_css_active%>" />
                            <%
